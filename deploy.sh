...
download_code() {
  ...
  # 使用代理下载
  PROXY_DOMAIN="gh.con.sh"
  REPO_URL="https://github.com/rdone4425/github-proxy/archive/refs/heads/main.zip"
  DOWNLOAD_URL="https://$PROXY_DOMAIN/$REPO_URL"
  
  # 下载代码
  info "使用代理下载: $DOWNLOAD_URL"
  if ! curl -L -o github-proxy.zip "$DOWNLOAD_URL"; then
    warning "代理下载失败，尝试直接从GitHub下载..."
    if ! curl -L -o github-proxy.zip "$REPO_URL"; then
      error "下载失败，请检查网络连接"
      exit 1
    fi
  fi
